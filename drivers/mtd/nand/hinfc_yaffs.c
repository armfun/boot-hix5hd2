...
static unsigned int hinfc610_support_yaffs2[] = {
	SET_HINFC_VER(HINFC_VER_610, _2K, hinfc610_ecc_none),
	SET_HINFC_VER(HINFC_VER_610, _4K, hinfc610_ecc_none),
	SET_HINFC_VER(HINFC_VER_610, _8K, hinfc610_ecc_none),
	SET_HINFC_VER(HINFC_VER_610, _16K, hinfc610_ecc_none),
	SET_HINFC_VER(HINFC_VER_610, _32K, hinfc610_ecc_none),
	0,
};
/*****************************************************************************/
		...
		unsigned int length)
{
	...
	if (host_info->hostver == HINFC_VER_300 ||
		...
		host_eccreg = hinfc504_ecc_type2reg(host_info->ecctype);
		yaffs2_tag_ecctype = hinfc504_ecc_reg2type(yaffs2_tags->eccreg);
	} else
	...
	if (host_info->hostver == HINFC_VER_610) {
		...
		host_eccreg = hinfc610_ecc_none;
		yaffs2_tag_ecctype = hinfc610_ecc_reg2type(yaffs2_tags->eccreg);
	} else
	...
	if (!host_merge_ver) {
		printf("!!! The yaffs2 filesystem "
			...
			yaffs2_tags->nandip, host_info->hostver);
		goto fail1;
	}

	yaffs2_merge_ver = get_yaffs2_version(yaffs2_tags->nandip,
		writesize, host_eccreg);
	if (!yaffs2_merge_ver) {
		printf("!!! The yaffs2 filesystem image"
