...
static int hinfc610_onfi_support_sync(struct hinfc_host *host)
{
	...
	/* no need to config WAIT_READY_EN */
...
static int hinfc610_get_onfi_info(struct hinfc_host *host, int *type)
{
	...
	*type = 0;
	...
	hinfc_write(host, HINFC610_READ_1CMD_1ADD_DATA_WAIT_READY, HINFC610_OP);
...
static int hinfc610_toggle_support_sync(struct hinfc_host *host)
{
	...
	// TODO: toggle ddr/sync ddr µÄÇø±ð.
	return 1;
}
/*****************************************************************************/
	...
	*type = 0;
	...
	if (buf[4] & (1 << 1))
		...
		*type = NAND_TYPE_TOGGLE_10;
		...
		printk("WARNING: sync NAND has unknown toggle revision. \n");
